...
export default class StepSeries {
	...
	constructor(id: number, osc: Osc, config?: any, stepAngle: number = 1.8) {
		...
		this.homeSwMode = this.#_homeSwMode;
	}
	...
	set homeSwMode(value: 0 | 1) {
		...
		this.osc.send(this.host, this.port, "/setHomeSwMode", [this.id, this.#_homeSwMode]);
	}

	getHomeSwMode = (): void => {
		this.osc.send(this.host, this.port, "/getHomeSwMode", [this.id]);
	};
